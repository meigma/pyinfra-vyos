...
**Empty-stdout guard**: pyinfra skips `process()` entirely on empty stdout, silently returning `default()` — so every fact command emits a package-controlled trailing marker on success, forced onto its own line with `printf '\n%s\n' <marker>` (a bare `echo` could concatenate onto payload lacking a final newline); the real command's rc propagates through `&&`. `process()` requires and strips the trailing marker before parsing. A successful-but-empty payload now fails loudly through the parser instead of masquerading as "no vbash".
